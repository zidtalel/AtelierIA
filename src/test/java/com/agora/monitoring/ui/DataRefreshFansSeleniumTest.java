...
public class DataRefreshFansSeleniumTest {
    ...
    @Test
    public void DataRefreshFans() throws Exception {
    ...
    driver.get(base);
        ...
        wait.until(ExpectedConditions.presenceOfElementLocated(org.openqa.selenium.By.cssSelector("table#fans")));

        // Capture initial state (may be empty)
        int initialRows = page.getFansRows().size();

        // Wait for ~6 seconds to allow an automatic refresh cycle (5s) to occur
        Thread.sleep(6500);

        int afterRows = page.getFansRows().size();
